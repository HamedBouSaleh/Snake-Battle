...
class InputBox:
    ...
    def draw(self, surf):
        ...
        if self.active and (pygame.time.get_ticks() // 500) % 2 == 0:
            ...
            cy = self.rect.y + self.rect.h//2 - txt.get_height()//2
            pygame.draw.line(surf, WHITE, (cx, cy), (cx, cy + txt.get_height()), 2)
...
def draw_game_screen(surf, state, my_name, p1, p2,
    ...
    for pname, sdata in state.get("snakes", {}).items():
        ...
        col     = tuple(raw_col) if raw_col else (
            SNAKE_COLORS["player1"] if pname == p1 else SNAKE_COLORS["player2"])
...
_L_X  = 50    # left column x
_L_W  = 390   # left column player-row width
_R_X  = 465   # right column x
_R_W  = 420   # right column width
_ROW_H = 44
_ROW_GAP = 52
_LIST_Y = 155  # y of first row

def _lobby_player_rect(i):
    return pygame.Rect(_L_X, _LIST_Y + i*_ROW_GAP, _L_W, _ROW_H)

def _lobby_watch_rect(i):
    # WATCH button inside the right-column game row
...
def draw_lobby(surf, font_sm, font_med, font_lg,
    ...
    # Challenge button / waiting indicator
...
def draw_map_picker(surf, font_sm, font_med, font_lg, selected_theme):
    ...
    for i, (tkey, tcfg) in enumerate(THEMES.items()):
        ...
        for c in range(0, pw, 16):
            pygame.draw.line(surf, tcfg["line_col"], (px+c, py), (px+c, py+ph), 1)
        for r in range(0, ph, 16):
            pygame.draw.line(surf, tcfg["line_col"], (px, py+r), (px+pw, py+r), 1)
        sel = tkey == selected_theme
        bc  = ACCENT if sel else (ACCENT2 if pygame.Rect(px, py, pw, ph).collidepoint(mx, my) else GRAY)
        pygame.draw.rect(surf, bc, (px, py, pw, ph), 3 if sel else 1)
        lbl = font_sm.render((">> " if sel else "") + tcfg["label"], True,
...
def draw_custom_screen(surf, font_sm, font_med, font_lg, my_color, my_hat):
    ...
    btn = pygame.Rect(300, 460, 300, 48)
    draw_button(surf, btn, "NEXT: CHOOSE MAP", font_med, hover=btn.collidepoint(mx2, my2))
...
def draw_end(surf, font_sm, font_med, font_lg,
    ...
    draw_button(surf, btn, "BACK TO LOBBY", font_sm, hover=btn.collidepoint(mx, my))
...
class App:
    def __init__(self):
        ...
        self.inp_user = InputBox(CX, 390, IW, IH, placeholder="e.g. snake_king")
        ...
        self.challenge_sent_to  = ""    # username we challenged (waiting for accept)
    ...
    def _process_network(self):
        for msg in self.net.poll():
            ...
            if t == "join_ok":
                ...
                # Lobby players: active_games list will update via next lobby broadcast
                ...
                # Either the challenger withdrew or the target disconnected
                by = msg.get("by", "")
